Упрощает маппинг SQL-запросов на DTO с использованием [Doctrine](https://github.com/doctrine/orm)
...
Решаемые проблемы:
  * Избежание [N+1](https://stackoverflow.com/questions/97197/what-is-the-n1-selects-problem-in-orm-object-relational-mapping)
    запросов. Для связей можно указать `fetch="EAGER"`, но такие связи всегда будет подгружаться, даже когда это не нужно
  ...
  * Сократить количество данных в SELECT, Doctrine всегда подставляет все имеющиеся столбцы в запрос. Можно переместить
    ...
    * Сами по себе трейты некоторые считают антипаттерном
...
Описание реляционной модели:
  ...
  * `User` one-to-one `Profile` поля `firstName`, `secondName` и `email`
  * `User` many-to-one `City` поле `name`
  * `City` one-to-many `News` поля `title` и `link`
...
Типа **возвращаемых значений** выводятся с помощью дженериков на основании имени класса аспекта.
