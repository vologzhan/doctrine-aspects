Маппинг SQL запроса на DTO с использованием [Doctrine](https://github.com/doctrine/orm)

<details>
<summary>Решаемые проблемы</summary>
...
Рассмотрим на примере email рассылки с новостями города для пользователя
...
Добавляется к классу DTO для связи с Entity. Аннотация нужна только при вызове методов [DtoMapper](src/DtoMapper.php)
...
В запросе в `SELECT` используется просто `*`, маппер на основе DTO сам выберет какие колонки должны быть в выборке.
